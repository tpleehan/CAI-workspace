...
public class ReplyController {
	...
	public Map<String, Object> getList(@PathVariable int bno, @PathVariable int pageNum) {
		...
		/*
		 * 1. getList 메서드는 (글번호, 페이지번호)를 매개값으로 받는다.
		 * 2. Mapper 인터페이스에 각각 다른 값을 전달하기 위해 Map 또는 @Param 어노테이션 사용
		 * 3. ReplyMapper.xml에 sql문을 페이징 쿼리로 작성
		 * 4. Rest 방식은 화면에 필요한 값을 여러개 보낼 때, 리턴에 Map이나 VO형식으로 필요한 데이터를 한번에 담아서 처리
		 * 댓글 목록 리스트, 전체 댓글 개수를 Map에 담아서 전달 
		 */
		
		PageVO vo = new PageVO();
		vo.setPageNum(pageNum); // 화면에서 전달된 페이지 번호
		vo.setCountPerPage(5); // 한 화면에 댓글 5개씩
