<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%--
		...
		- 나머지는 "당신은 정상체중입니다." 를 출력
		  
		주의!) 
		request.getParameter()의 리턴 타입은 String
		숫자가 전달되어도 타입이 문자열이라서
		bmi 지수를 계산하려면 실수나 정수로 변환 후 계산해야 한다.
		Integer.parseInt(문자열), Double.parseDouble(문자열)
		신장 -> cm, 체중 -> kg 두 값 모두 소수점 포함 값으로 받도록 한다.
	--%>
    <%
		// 받아올 파라미터값 실수로 변환
		...
		//double cm = Double.parseDouble(request.getParameter("cm")); 이렇게 사용해도 된다.
