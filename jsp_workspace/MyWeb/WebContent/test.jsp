...
<body>
	...
	<%--
		for (int i = 1; i <= 300; i++) {
			...
			String content = "테스트 진행 중입니다. 차후에 공지드리겠습니다." + i;
	...
	<%
		out.print("-------------------------------------<br>");
		out.print("# 총 게시물 수: " + BoardDAO.getInstance().countAticles() + "<br>");
		out.print("-------------------------------------<br>");
		...
		// 다음 버튼 활성화 여부
		boolean isNext = (BoardDAO.getInstance().countAticles() <= (endPage * paging.getCountPerPage())) ? false : true;
		out.print("다음 버튼 활성화 여부: " + isNext + "<br>");
		
		// 끝 페이지 보정
		if(!isNext) { // isNext == false
			endPage = (int) Math.ceil(BoardDAO.getInstance().countAticles() / (double) paging.getCountPerPage());
		}
