...
function inputNumber() {
  ...
  // 객체 디스트럭쳐링(ES6): 데이터 참조시 사용 가능 수정, 추가시 사용 불가.
...
function checkContDown(countDown) {
  if (countDown >= 0) {
    alert('You Win');
  } else {
    alert('You Lose');
...
function alertCountDown(countDown) {
  ...
  if (countDown > 0) {
    alert('정답 기회 ' + countDown + '번 남았습니다.');
  } else {
    alert('정답 기회가 모두 소진되었습니다. 문제는 계속 풀어주세요.');
  }
