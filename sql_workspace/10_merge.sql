...
/*
MERGE 테이블 병합
UPDATE와 INSERT를 한번에 처리

한 테이블에 해당하는 데이터가 있으면 UPDATE 없으면 INSERT로 처리.

만약 MERGE가 없다면
해당 데이터의 존재 유무를 일일이 확인해야 하고
if문을 사용해서 데이터가 있으면 UPDATE, 없으면 else문 사용해서
INSERT를 처리하도록 진행해야 하는데 MERGE를 통해 쉽게 처리 가능
*/
...
WHEN MATCHED THEN -- 조건에 일치할 경우 타켓 테이블에 실행
    ...
    DELETE WHERE ~~~
        ...
        삭제할 대상 컬럼들을 동일한 값으로 먼저 UPDATE를 진행하고
        DELETE의 WHERE절에 지정한 동일한 값을 지정해서 삭제한다.
    */
...
-- 기존의 데이터는 email, phone, salary, commission, man_id, dept_id는 업데이트 하도록 처리
-- 새로 유입된 데이터는 그대로 추가.
