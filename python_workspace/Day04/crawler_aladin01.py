# cmd -> pip install bs4 (BeautifulSoup 라이브러리 다운로드)
...
# 뷰티풀수프 객체 생성
# 뷰티풀수프 객체를 생성하면서, 셀레늄이 가지고 온 html 소스코드를 제공하고, 해당 소스코드를 html 문법으로 변환
soup = BeautifulSoup(src, 'html.parser')

'''
- 뷰티풀수프를 사용해서 수집하고 싶은 데이터를 골라 그 안에 들어있는 태그를 부분 추출할 수 있다.
- find_all() 메서드는 인수값으로 추출하고자 하는 태그의 이름을 적으면 해당 태그만 전부 추출해서 리스트에 담아 대입한다.
'''
div_list = soup.findAll('div', class_='ss_book_box')
# print('div_list에 들어 있는 데이터 수:', len(div_list))
# print(div_list[0]) # 베스트셀러 1위 책만 가져오기

first_book = div_list[0].find_all('li')
# print(first_book) -> li안에 필요한 텍스트를 확인할 수 있다.

# text는 태그를 제외한 사용자가 실제로 브라우저에서 확인 가능한 텍스트만을 추출하여 문자열 형태로 반환한다.
