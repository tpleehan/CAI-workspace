...
ex) def singup_user(id, pw, name, addr, email, phone, ...)

- 파이썬에서는 순서와 무관하게 인수를 전달할 수 있는 방법을 제공하기 때문에
 인수의 이름을 직접 지정해서 값을 전달하는 키워드 인수 방식을 제공한다.
'''
...
# 위치 인수와 키워드 인수의 혼합 사용 시 무조건 위치 인수가 앞에 와야 한다.
